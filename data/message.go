...
func (m *SMTPMessage) Parse(hostname string) *Message {
	...
	if msg.Content.IsMIME() {
		logrus.Tracef("Parsing MIME body")
	...
	if hdr, ok := content.Headers["Content-Type"]; ok {
		if len(hdr) > 0 {
			...
			if len(boundary) > 0 {
				...
				logrus.Tracef("Got boundary: %s", boundary)
			} else {
				logrus.Tracef("Boundary not found: %s", hdr[0])
			}
			...
			for _, s := range p {
				if len(s) > 0 {
					...
					if part.IsMIME() {
						logrus.Tracef("Parsing inner MIME body")
...
func ContentFromString(data string) *Content {
	logrus.Tracef("Parsing Content from string: '%s'", data)
	...
	if len(x) == 2 {
		...
		for _, hdr := range hdrs {
			if lastHdr != "" && (strings.HasPrefix(hdr, " ") || strings.HasPrefix(hdr, "\t")) {
				...
				logrus.Warnf("Found invalid header: '%s'", hdr)
			}
		}
		return &Content{
			Size:    len(data),
