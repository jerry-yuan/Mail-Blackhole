...
func createAPIv2(conf *config.APIConfig, msgStorage storage.Storage, r *pat.Router) *APIv2 {
	logrus.Println("Creating API v2 with WebPath: " + conf.WebPath)
	apiv2 := &APIv2{
		config:      conf,
		storage:     msgStorage,
	...
	go func() {
		for {
			...
			case msg := <-apiv2.messageChan:
				logrus.Println("Got message in APIv2 websocket channel")
				apiv2.broadcast(msg)
			}
		}
	}()
...
func (apiv2 *APIv2) messages(w http.ResponseWriter, req *http.Request) {
	logrus.Println("[APIv2] GET /api/v2/messages")
	...
	messages, err := apiv2.storage.List(start, limit)
	if err != nil {
		panic(err)
	}
...
func (apiv2 *APIv2) search(w http.ResponseWriter, req *http.Request) {
	logrus.Println("[APIv2] GET /api/v2/search")
	...
	messages, total, _ := apiv2.storage.Search(kind, query, start, limit)
...
func (apiv2 *APIv2) websocket(w http.ResponseWriter, req *http.Request) {
	logrus.Println("[APIv2] GET /api/v2/websocket")

	apiv2.wsHub.Serve(w, req)
}

func (apiv2 *APIv2) broadcast(msg *data.Message) {
	logrus.Println("[APIv2] BROADCAST /api/v2/websocket")
