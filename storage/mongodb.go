...
func CreateMongoDB(uri, db, coll string) (*MongoDB, error) {
	logrus.New().Tracef("Connecting to MongoDB: %s\n", uri)
	...
	if err != nil {
		return nil, fmt.Errorf("failed creating index: %s", err)
	}
	return &MongoDB{
		Session:    session,
...
func (mongo *MongoDB) Store(m *data.Message) (string, error) {
	...
	if err != nil {
		logrus.Printf("Error inserting message: %s", err)
		return "", err
	}
...
func (mongo *MongoDB) Search(kind, query string, start, limit int) (*data.Messages, int, error) {
	...
	if err != nil {
		logrus.Printf("Error loading messages: %s", err)
		return nil, 0, err
	}
...
func (mongo *MongoDB) Load(id string) (*data.Message, error) {
	...
	if err != nil {
		logrus.Printf("Error loading message: %s", err)
		return nil, err
	}
	return result, nil
}
